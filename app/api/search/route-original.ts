...
// Import enhanced configurations first to override all defaults
import '@/lib/enhanced-fetch';
import '@/lib/socket-config';
import '@/libexport async function POST(req: Request) {
  console.log('🚨 API ROUTE HIT - THIS SHOULD APPEAR IN LOGS');
...
const httpAgent = new http.Agent({
  keepAlive: true,
  timeout: 240000, // 4 minutes - match enhanced-fetch timeout
...
export const maxDuration = 300; // 5 minutes (reduced from 6 minutes)
...
export async function POST(req: Request) {
  ...
  /* TEMPORARILY COMMENTED OUT FOR TESTING
    ...
    console.log('🔍 Geolocation extracted successfully');
    ...
    // Wrap all database operations with timeouts to prevent hanging
    const dbTimeout = (promise: Promise<any>, timeoutMs: number, operation: string) => {
      return Promise.race([
        promise,
    ...
    const user = await dbTimeout(getCurrentUser(), 5000, 'getCurrentUser').catch(error => {
    ...
    // All models are now freely accessible without authentication
    ...
    const customInstructionsPromise = user ? getCachedCustomInstructions(user).catch(error => {
      console.error('Custom instructions failed:', error);
      return null;
    ...
    if (user) {
      ...
      try {
        ...
        const existingChat = await dbTimeout(getChatById({ id }), 3000, 'getChatById').catch(error => {
        ...
        if (!existingChat) {
          // Try to create chat, but don't block if it fails
          const saveChatStartTime = Date.now();
          console.log('🔍 [DB] Starting saveChat()...');
          await dbTimeout(saveChat({
            id,
            userId: user.id,
            title: 'New Chat', // Temporary title
            visibility: selectedVisibilityType,
          }), 3000, 'saveChat').catch(error => {
        ...
        // Create stream record as early as possible for resumable streaming (optional)
        const createStreamStartTime = Date.now();
        console.log('🔍 [DB] Starting createStreamId()...');
        await dbTimeout(createStreamId({ streamId, chatId: id }), 3000, 'createStreamId').catch(error => {
          console.error('createStreamId failed:', error);
      ...
      if (!isProUser) {
        ...
        try {
          ...
          const [messageCountResult, extremeSearchUsage] = await Promise.all([
            dbTimeout(getUserMessageCount(user), 3000, 'getUserMessageCount').catch(error => {
              console.error('getUserMessageCount failed:', error);
              return { count: 0, error: null };
            }),
            dbTimeout(getExtremeSearchUsageCount(user), 3000, 'getExtremeSearchUsageCount').catch(error => {
              console.error('getExtremeSearchUsageCount failed:', error);
              return { count: 0, error: null };
            }),
          ]);
    ...
    const configPromise = dbTimeout(getGroupConfig(group), 3000, 'getGroupConfig').then((config) => {
      ...
      console.error('getGroupConfig failed:', error);
    ...
    const stream = createUIMessageStream<ChatMessage>({
      execute: async ({ writer: dataStream }) => {
        try {
          ...
          const criticalResult = await Promise.race([
            criticalChecksPromise,
            new Promise((_, reject) => 
              setTimeout(() => reject(new Error('Critical checks timed out after 5 seconds')), 5000)
            )
          ]);
          console.log(`⏱️  Critical checks wait took: ${((Date.now() - criticalWaitStartTime) / 1000).toFixed(2)}s`);

          if (criticalResult && typeof criticalResult === 'object' && 'canProceed' in criticalResult) {
            if (!(criticalResult as any).canProceed) {
              throw (criticalResult as any).error;
            }
          }
        } catch (error) {
          console.error('Critical checks failed:', error);
          // Don't throw error immediately, continue with degraded service
          console.log('⚠️  Continuing with degraded service...');
        }
        ...
        const configResult = await Promise.race([
          ...
          new Promise((_, reject) => 
            setTimeout(() => reject(new Error('Config wait timed out after 5 seconds')), 5000)
          )
        ]).catch(error => {
          console.error('Config wait failed:', error);
          // Return defaults
          return [
            { tools: [], instructions: 'You are a helpful AI assistant.' },
            null
          ];
        }) as [any, any];
        const [{ tools: activeTools, instructions }, customInstructionsResult] = configResult;
        ...
        console.log(`🔄 Available tools: ${activeTools.length} tools`);
        ...
        try {
          result = streamText({
            ...
            abortSignal: AbortSignal.timeout(240000), // 4 minutes abort signal (reduced from 6.6 minutes)
            ...
            onError(event) {
              console.log('❌ StreamText Error: ', event.error);
              console.log('❌ Error type:', event.error?.constructor?.name);
              ...
              if (errorMessage?.toLowerCase().includes('timeout')) {
                console.log('🕐 Detected timeout error - this is likely a connection issue with the AI provider');
              }
            },
          });
      ...
      if (streamError instanceof Error) {
        ...
        if (errorMessage.includes('timeout') || errorMessage.includes('aborted')) {
          throw new ChatSDKError(
            'bad_request:api',
            'The AI service request timed out. This may be due to high server load. Please try again with a shorter message or try again later.'
          );
        }
      ...
      // Simply throw the error to be handled by the outer error handler
      throw new ChatSDKError(
        'bad_request:api',
        streamError instanceof Error && streamError.message.toLowerCase().includes('timeout')
          ? 'The AI service is experiencing timeouts. Please try again in a moment.'
      ...
      onError(error) {
        console.log('🔥 UIMessageStream Error: ', error);
        ...
        if (errorMessage.toLowerCase().includes('timeout')) {
          return 'The AI service is experiencing timeouts. Please try again in a moment.';
        }
  ...
  clearTimeout(requestTimeout); // Clear timeout on any error
